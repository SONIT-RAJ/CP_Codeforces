...
ll divv(ll a, ll b, ll m) {a = a % m; b = b % m; return (mod_mul(a, mminvprime(b, m), m) + m) % m;}  //only for prime m
...
int dfs(vector<vector<int>>&adj,int i,int p,vector<int>&c,vector<int>&d,int &ans){
    int present=c[i];
    for(auto &x:adj[i]){
        if(x==p)continue;
        present+=dfs(adj,x,i,c,d,ans);
    }
    if(present<d[i]){
        ans=0;
        return 0;
