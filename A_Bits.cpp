...
void solve(){
    ...
    int l,r;
    cin>>l>>r;
    string loww=decToBinary(l);
    string high=decToBinary(r);
    int n=high.size();
    int m=loww.size();
    string low;
    int req=n-m;
    ...
    for(int i=0;i<m;i++){
        low.push_back(loww[i]);
    }
    string ans;
    int stight=1;
    int btight=1;
    for(int i=0;i<n;i++){
        if(stight==0 && btight==0){
            ans.push_back('1');
        }
        else if(stight==0){
            if(high[i]=='1'){
    ...
    int val=binaryToDecimal(ans);
    if(ppcll(val)<ppcll(r)){
        val=r;
    }
    cout<<val;
