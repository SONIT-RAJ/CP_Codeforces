...
bool check(int up,int down,int a,int b){
    int mini=min(a,b);
    a-=mini;
    b-=mini;
    int d=max(a,b);
    ...
    if(up>down){
        if(a==d){
            ...
            if((b/2)*9==up-down && d%2==0)return true;
            return false;
        }
    }
    return true;
...
void solve(){
    ...
    bool flag1=check(left,right,a,b);
    bool flag2=check(right,left,b,a);
    if(flag1 && flag2){
        cout<<"Bicarp";
    }
    else{
