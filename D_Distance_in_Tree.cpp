...
// int dp[max_n];
void dfs(int node,int parent,vector<vector<int>>&a,vector<vector<int>>&dp,int &k){
    for(auto &x:a[node]){
        if(x!=parent){
            dfs(x,node,a,dp,k);
            for(int i=1;i<=k;i++){
...
void dfs2(int node,int parent,vector<vector<int>>&ans,vector<vector<int>>&a,vector<vector<int>>&dp,int &k){
    for(auto &x:a[node]){
        if(x!=parent){
            dfs2(x,node,ans,a,dp,k);
            for(int i=1;i<k;i++){
                ans[node][k]+=dp[x][i-1]*(dp[node][k-i]-dp[x][k-i-1]);
            }
        }
    }
}
...
void solve(){
    ...
    vector<vector<int>>dp(n+1,vector<int>(k+1,0));// node ,distance of k in that sub tree
    for(int i=1;i<=n;i++){
        dp[i][0]=1;
    }
    dfs(1,0,a,dp,k);
