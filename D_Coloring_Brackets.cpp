...
int f(int i,int j,int ci,int cj,vector<vector<vector<vector<int>>>>&dp,string &s,vector<int>&a){
    if(dp[i][j][ci][cj]!=-1)return dp[i][j][ci][cj];
    int ans=0;
    if(a[i]==j){
        if(ci!=0 && cj!=0){
            return dp[i][j][ci][cj]=0;
        }
        if(ci==cj){
            return dp[i][j][ci][cj]=0;
        }
        if(i+1==j){
            return dp[i][j][ci][cj]=1;
        }
        for(int left=0;left<3;left++){
            for(int right=0;right<3;right++){
                if(ci!=0 && left==ci)continue;
                if(cj!=0 && right==cj)continue;
                ans=(ans+f(i+1,j-1,left,right,dp,s,a))%mod;
            }
        }
    }
    else{
        ...
        for(int left=0;left<3;left++){
            for(int right=0;right<3;right++){
                if(ci==right || (ci!=0 && right!=0))continue;
                if(left!=0 && (left==right))continue;
                int temp=(f(i,new_j,ci,right,dp,s,a)*f(new_i,j,left,cj,dp,s,a))%mod;
                ans=(ans+temp)%mod;
            }
        }
    }
    return dp[i][j][ci][cj]=ans;
}
...
void solve(){
    ...
    vector<vector<vector<vector<int>>>>dp(n,vector<vector<vector<int>>>(n,vector<vector<int>>(3,vector<int>(3,-1))));
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            ans=(ans+f(0,n-1,i,j,dp,s,a))%mod;
        }
    }
    cout<<ans;
