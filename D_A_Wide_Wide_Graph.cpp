...
void dfs(vector<vector<int>>&a,int i,int p,int dis,int &maxi,int &d1){
    if(dis>maxi){
        maxi=dis;
        d1=i;
    }
    for(auto &x:a[i]){
        if(x==p)continue;
        dfs(a,x,i,dis+1,maxi,d1);
    }
}
void dfs2(vector<vector<int>>&a,int i,int p,int dis,map<int,vector<int>>&mpp,int &maxi,int &d1){
    mpp[dis].push_back(i);
    if(dis>maxi){
        maxi=dis;
        d1=i;
...
void solve(){
    ...
    maxi=0;
    int d2=d1;
    dfs2(a,d1,0,0,mpp,maxi,d2);

    map<int,vector<int>>mpp2;
    maxi=0;
    int d3=1;
    dfs2(a,d2,0,0,mpp2,maxi,d3);
