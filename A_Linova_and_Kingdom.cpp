...
// int dp[max_n];
int dfs(vector<int>a[],vector<int>&dp,int i,int p){
    int ans=1;
    for(auto &x:a[i]){
        if(x!=p){
            ans+=dfs(a,dp,x,i);
...
int ans_dfs(vector<int>a[],vector<int>&v,int i,int p,int &answer){
    int ans=0;
    for(auto &x:a[i]){
        if(x!=p){
            ans+=ans_dfs(a,v,x,i,answer);
        }
    }
    if(v[i]==1){
        answer+=ans;
...
void solve(){
    ...
    queue<vector<int>>q;//node,parent,level
    q.push({1,0,0});
    while(!q.empty()){
