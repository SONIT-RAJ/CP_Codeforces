...
void dfs1(vector<vector<vector<int>>>&a,int i,vector<bool>&v,int &val,vector<char>&m){
    val++;
    v[i]=true;
    for(auto &x:a[i]){
        int t=x[0];
...
void dfs2(vector<vector<vector<int>>>&a,int i,vector<bool>&v,int &val,vector<char>&m){
    val++;
    v[i]=true;
    for(auto &x:a[i]){
        int t=x[0];
...
void solve(){
    ...
    for(int i=0;i<m;i++){
        ...
        int idx=e[i][3];
        if(t==2){
            a[u].push_back({2,v,idx,0});
        }
        else{
            a[u].push_back({1,v,idx,0});
            a[v].push_back({1,u,idx,1});
        }
    }
    int mini=0;
    vector<char>minn(m,'a');
    vector<bool>v(n+1,false);
    dfs1(a,s,v,mini,minn);

    int maxi=0;
    vector<char>maxx(m,'a');
    vector<bool>v2(n+1,false);
    dfs2(a,s,v2,maxi,maxx);

    cout<<maxi<<endl;
    for(int i=0;i<m;i++){
        if(ee[i][0]==2)continue;
        if(maxx[i]=='a'){
            cout<<'-';
            continue;
        }
