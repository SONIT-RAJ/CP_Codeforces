...
void solve(){
    ...
    int n,m;
    cin>>n>>m;
    // dp[i][a][b] = ways to make pair of array upto ith index if ith elements of array A and B are a and b respectively
    // ans = dp[m][1][1]+dp[m][1][2]...+dp[m][1][n]
    //      +dp[m][2][2]+dp[m][2][3]...+dp[m][2][n]
    //      +dp[m][n][n]
    // base case dp[0][0][b]=1
    //           dp[0][a][0]=0 (a>0)
    //           dp[0][a][n]=1
    // dp[i][a][b] = if(a>b)? 0 : sum of all dp[i-1][j][k] where j<=a && k>=b
    vector<vector<vector<int>>>dp(2,vector<vector<int>>(n+2,vector<int>(n+2,0)));
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
            if(i<=j){
                dp[0][i][j]=1;
    ...
    for(int i=1;i<=n;i++){
        for(int j=n;j>=1;j--){
            p[i][j]=(p[i-1][j]+p[i][j+1]-p[i-1][j+1]+dp[0][i][j]+mod)%mod;
        }
    }
    for(int i=1;i<m;i++){
        for(int a=1;a<=n;a++){
        ...
        for(int a=0;a<=n+1;a++){
            for(int b=n+1;b>=0;b--){
                int up=(a-1<0)?0:p[a-1][b];
                int left=(b+1>n+1)?0:p[a][b+1];
                int cornor=(a-1<0 || b+1>n+1)?0:p[a-1][b+1];
                p[a][b]=(up+left-cornor+dp[i%2][a][b]+mod)%mod;
            }
        }
    }
    int ans=0;
