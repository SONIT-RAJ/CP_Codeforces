...
bool check(vector<vector<int>>&b,vector<int>&left,int c,int n,int mid){
    int req=0;
    bool lefty=false;
    for(int i=0;i<mid;i++){
        req+=b[i][0];
        if(b[i][1]==0)lefty=true;
    }
    if(req>c)return false;
    if(lefty==true)return true;
    int e=1e18;
    for(int i=0;i<mid;i++){
        int idx=b[i][2];
        e=min(e,left[idx]-b[i][0]);
    }
    if(req+e<=c)return true;
    int mini=1e18;
    for(int i=mid;i<n;i++){
        int idx=b[i][2];
        mini=min(mini,left[idx]);
    }
    if(req+mini-b[mid-1][0]<=c)return true;
    return false;
}
