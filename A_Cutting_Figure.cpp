...
void dfs(vector<string>&a,int i,int j,vector<vector<int>>&d,int r,int c,int &cnt,vector<vector<int>>&v){
    v[i][j]=1;
    cnt++;
    for(int k=0;k<4;k++){
        int new_i=i+d[k][0];
        int new_j=j+d[k][1];
        if(new_i>=0 && new_j>=0 && new_i<r && new_j<c && v[new_i][new_j]==-1 && a[new_i][new_j]=='#'){
            dfs(a,new_i,new_j,d,r,c,cnt,v);
        }
    }
}
    ...
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
            if(a[i][j]=='#'){
                ...
                v[i][j]=1;
                if(n1==i && n2==j){
                    dfs(a,n3,n4,d,r,c,cnt,v);
                }
                else{
                    dfs(a,n1,n2,d,r,c,cnt,v);
                }
...
// 1. iF the number of colored cells are <=2 then answer is -1
// the answer can be max 2.
//two cases answer=1 and answer=2