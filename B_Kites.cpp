...
void solve(){
    ...
    left[0]=1e18;
    for(int i=1;i<n;i++){
        left[i]=min(d[i],left[i-1]);
    }
    vector<int>right(n+1);
    right[n]=1e18;
    for(int i=n-1;i>=1;i--){
        right[i]=min(d[i],right[i+1]);
    }
    int ans=1e18;
    for(int i=1;i<n;i++){
        int lefty=(i-2<0)?1e18:left[i-2];
        int righty=(i+2>n)?1e18:right[i+2];
        ans=min(ans,d[i]+min(lefty,righty));
    }
    cout<<ans;
