...
void solve(){
    ...
    for(int i=0;i<n;i++){
        int x=a[i];
        dp1[x+1]=(dp1[x+1]*2+dp1[x])%M;
        if(x>0)dp2[x-1]=(dp2[x-1]*2+dp1[x-1])%M;
        dp2[x+1]=(dp2[x+1]*2)%M;
    }
    int ans=0;
    for(int i = 1; i < n + 1; i++) ans = (ans + dp1[i]) % M;
    for(int i = 0; i < n + 1; i++) ans = (ans + dp2[i]) % M;
    cout<<ans;
