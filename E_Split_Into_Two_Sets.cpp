...
// int dp[max_n];
bool dfs(vector<int>adj[],vector<int>&v,int node,int val){
    v[node]=val;
    for(auto &x:adj[node]){
        if(v[x]==-1){
            if(dfs(adj,v,x,1-val)){
                return true;
            }
        }
        else if(v[x]==val){
...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        ...
        if(mpp.find(u)!=mpp.end()){
            adj[mpp[u]].push_back(i);
            adj[i].push_back(mpp[u]);
        }
        else{
            mpp[u]=i;
        }
        if(mpp.find(v)!=mpp.end()){
            adj[mpp[v]].push_back(i);
            adj[i].push_back(mpp[v]);
        }
        else{
            mpp[v]=i;
        }
        if(f[u]>2 || f[v]>2)flag=true;
    }
    if(flag){
        cout<<"NO";
        return;
