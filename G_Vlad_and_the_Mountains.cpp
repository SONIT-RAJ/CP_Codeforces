...
void solve(){
    ...
    for(int i=0;i<m;i++){
        cin>>a[i][0]>>a[i][1];
        a[i][2]=max(b[a[i][0]],b[a[i][1]]);
    }
    int sz;
    cin>>sz;
    vector<vector<int>>q(sz,vector<int>(5));
    for(int i=0;i<sz;i++){
    ...
    for(int i=0;i<sz;i++){
        int val=q[i][4];
        while(idx<m && a[idx][2]<=val){
            d.unite(a[idx][0],a[idx][1]);
            idx++;
        }
        int p1=d.find(q[i][0]);
        int p2=d.find(q[i][1]);
        if(p1==p2)ans[q[i][3]]=true;
    }
    for(int i=0;i<sz;i++){
        if(ans[i]){
            cout<<"YES"<<endl;
