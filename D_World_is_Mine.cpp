...
int f(vector<int>&a,map<int,int>&mpp,vector<vector<int>>&dp,int i,int j){
    if(i>=a.size()){
        return 0;
    }
    if(dp[i][j]!=-1)return dp[i][j];
    dp[i][j]=f(a,mpp,dp,i+1,j);
    int cnt=mpp[a[i]];
    if(cnt+j<=i){
        dp[i][j]=max(dp[i][j],1+f(a,mpp,dp,i+1,j+cnt+1));
    }
    return dp[i][j];
}
