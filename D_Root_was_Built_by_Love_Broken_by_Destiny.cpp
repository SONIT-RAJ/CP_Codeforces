...
bool dfs(vector<vector<int>>&a,vector<bool>&v,int i,int p){
    v[i]=true;
    for(auto &x:a[i]){
        if(x==p)continue;
        if(v[x]==true)return true;
        if(dfs(a,v,x,i))return true;
    }
    return false;
}
void dfs2(vector<vector<int>>&a,vector<int>&d,int i,int p,int &ans){
    int c=0;
    int e=(p==0)?0:1;
    for(auto &x:a[i]){
        if(x==p)continue;
    ...
    if(e==0){
        if(c==0){
            ...
            ans=(ans*f[d[i]-1-e])%mod;
            ans=(ans*2)%mod;
        }
        else if(c==2){
            ans=(ans*f[d[i]-2-e])%mod;
            ans=(ans*2)%mod;
        }
        else{
            ans=0;
