...
void solve(){
    ...
    map<int,int>mpp;
    vector<int>np,p;
    vector<int> sievee = sieve(1000000);
    for(int i=0;i<2*n;i++){
        cin>>a[i];
        mpp[a[i]]++;
        if(mpp[a[i]]==1){
    ...
    for(int i=sz-1;i>=0;i--){
        for(int j=0;j<=n;j++){
            if(j>0){
                dp[i][j]= (div(dp[i+1][j],fact[mpp[p[i]]],M)  + div(dp[i+1][j-1],fact[mpp[p[i]]-1],M))%M;
            }
            else{
                dp[i][j] = div(dp[i+1][j],fact[mpp[p[i]]],M);
            }
        }
    }
    cout<< (ans*dp[0][n])%M;
