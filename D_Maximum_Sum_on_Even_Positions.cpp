...
int f(vector<int>&a,vector<vector<int>>&dp,int i,int j){
    ...
    if(j==0){
        dp[i][j]=f(a,dp,i+1,j);
        if(i==0){
            dp[i][j]=max(dp[i][j],s*(a[i]-a[i+1])+f(a,dp,i+2,1));
        }
        else if(i+1>=a.size()){
            dp[i][j]=max(dp[i][j],s*(a[i]-a[i-1])+f(a,dp,i+1,1));
        }
        else{
            dp[i][j]=max(dp[i][j],s*(a[i]-a[i+1])+f(a,dp,i+2,1));
            dp[i][j]=max(dp[i][j],s*(a[i]-a[i-1])+f(a,dp,i+1,1));
        ...
        if(i+1>=a.size()){
            ...
            dp[i][j]=max(0ll,s*(a[i]-a[i+1])+f(a,dp,i+2,j));
        }
    }
    return dp[i][j];
}
