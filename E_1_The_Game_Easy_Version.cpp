...
void dfs(vector<vector<int>>&adj,vector<int>&entry,vector<int>&exit,int i,int p,int &time){
    entry[i]=time++;
    for(auto &x:adj[i]){
        if(x!=p){
            dfs(adj,entry,exit,x,i,time);
        }
    }
    exit[i]=time++;
}
...
void solve(){
    ...
    vector<int>exit(n+1);
    int time=0;
    dfs(adj,entry,exit,1,0,time);
    int low=LLONG_MAX;
    int high=LLONG_MIN;
    auto last=mpp.rbegin();
    auto temp=last->second;
    for(int i=0;i<temp.size();i++){
        int node=temp[i];
        low=min(low,entry[node]);
        high=max(high,exit[node]);
    }
    for(auto it=mpp.rbegin();it!=mpp.rend();it++){
        ...
        for(int i=0;i<v.size();i++){
            ...
            new_low=min(new_low,left);
            new_high=max(new_high,right);
        }
        low=new_low;
        high=new_high;
    }
