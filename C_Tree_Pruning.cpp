...
// int dp[max_n];
int dfs(vector<vector<int>>&adj,vector<int>&prefix,vector<int>&suffix,int node,int parent,int level){
    suffix[level]++;
    int new_level=level;
    for(auto &child:adj[node]){
        if(child!=parent){
            int max_depth=dfs(adj,prefix,suffix,child,node,level+1);
            new_level=max(new_level,max_depth);
        }
    }
    prefix[new_level]++;
    return new_level;
