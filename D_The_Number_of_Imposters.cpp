...
// int dp[max_n];
int dfs(vector<vector<pair<int,int>>>&a,vector<int>&v,int node,int p,int &n){
    int ans=0;
    v[node]=p;
    if(p==1){
        ans++;
    ...
    for(auto &x:a[node]){
        ...
        if(v[child]==-1){
            if(p==0){
                if(w==0){
                    ans+=dfs(a,v,child,0,n);
                }
                else{
                    ans+=dfs(a,v,child,1,n);
                }
            }
            else{
                if(w==0){
...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        if(v0[i]==-1 && v1[i]==-1){
            int val0=dfs(a,v0,i,0,n);
            int val1=dfs(a,v1,i,1,n);
            if(val0>n || val1>n){
                cout<<-1;
                return;
            }
