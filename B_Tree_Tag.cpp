...
void dfs(vector<vector<int>>&adj,int i,int p,int b,int &dis,int d){
    if(i==b){
        dis=d;
    }
    for(auto &x:adj[i]){
        if(p==x)continue;
        dfs(adj,x,i,b,dis,d+1);
    }
}

void dfs2(vector<vector<int>>&adj,int i,int p,int &dis,int d,int &node){
    for(auto &x:adj[i]){
        if(x==p)continue;
        dfs2(adj,x,i,dis,d+1,node);
    }
    if(dis<d){
        dis=d;
        node=i;
...
void solve(){
    ...
    int diameter=0;
    int node=1;
    dfs2(adj,a,0,dis,0,node);
    dfs2(adj,node,0,diameter,0,a);
    if(diameter>=2*da+1){
        cout<<"Bob";
    }
    else{
