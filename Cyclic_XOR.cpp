...
void solve(){
    ...
    cin>>n>>k;
    int jump_size=1<<k;
    int cycle=__gcd(jump_size,n);
    int length=n/cycle;
    int ways=(length%3==0)?4:1;
    int ans=power(ways,cycle,M);//for a single bit
    ans=power(ans,k,M);//for all bits in a number less than equal to 2^k
    cout<<ans;
