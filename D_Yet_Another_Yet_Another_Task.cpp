...
// int dp[max_n];
int f(vector<int>&a,vector<vector<vector<int>>>&dp,int i,int j,int maxi){
    if(i==a.size()){
        if(j==1) return 30-maxi;
        return -1e9;
    }
    if(dp[i][j][maxi]!=-1)return dp[i][j][maxi];
    if(j==0){
        dp[i][j][maxi]=max(f(a,dp,i+1,0,0),a[i]+f(a,dp,i+1,1,max(maxi,a[i]+30)));
    }
    else{
        dp[i][j][maxi]=max(30-maxi,a[i]+f(a,dp,i+1,1,max(maxi,a[i]+30)));
    }
    return dp[i][j][maxi];
}
...
void solve(){
    ...
    vector<int>a(n);
    cin>>a;
    vector<vector<vector<int>>>dp(n,vector<vector<int>>(2,vector<int>(61,-1)));
    cout<<f(a,dp,0,0,0);
