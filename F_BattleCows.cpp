...
void build(vector<int>&a,int i,int l,int r){
    ...
    pair<int,int> left=s[2*i+1];
    pair<int,int> right=s[2*i+2];
    int val=left.first^right.first;
    if(left.first>=right.first){
        s[i]={val,left.second};
    }
    else{
        s[i]={val,right.second};
    }
}
void range(vector<int>&a,int i,int l,int r,int idx,int new_skill){
    if(l==r){
        s[i]={new_skill,l};
        return;
    }
    int mid=l+(r-l)/2;
    if(idx<=mid){
        range(a,2*i+1,l,mid,idx,new_skill);
    }
    else{
        range(a,2*i+2,mid+1,r,idx,new_skill);
    }
    pair<int,int> left=s[2*i+1];
    pair<int,int> right=s[2*i+2];
    int val=left.first^right.first;
...
int query(vector<int>&a,int i,int l,int r,int idx){
    int w=s[i].second;
    int ans=0;
    if(w==idx){
        return 0;
    ...
    if(idx<=mid){
        if(w>mid){
            ans+=r-mid+query(a,2*i+1,l,mid,idx);
        }
        else{
            ans+=query(a,2*i+1,l,mid,idx);
        }
    }
    else{
        if(w<=mid){
            ans+=mid-l+1+query(a,2*i+2,mid+1,r,idx);
        }
        else{
            ans+=query(a,2*i+2,mid+1,r,idx);
        }
    }
    return ans;
}
...
void solve(){
    ...
    while(q--){
        ...
        cin>>idx>>new_skill;
        idx--;
        range(a,0,0,size-1,idx,new_skill);
        int ans=query(a,0,0,size-1,idx);
        range(a,0,0,size-1,idx,a[idx]);
        cout<<ans<<"\n";
    }
