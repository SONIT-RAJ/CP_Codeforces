...
void solve(){
    ...
    vector<int>a(n);
    cin>>a;
    vector<vector<vector<bool>>>dp(n,vector<vector<bool>>(k/2+1,vector<bool>(k+1,false)));
    for(int i=0;i<n;i++){
        dp[i][0][0]=true;
    }
    for(int i=0;i<n;i++){
        for(int x=0;x<=k/2;x++){
            for(int y=0;y<=k;y++){
                if(i-1<0){
                    ...
                    dp[i][x][y]=dp[i-1][x][y];
                    if(x>=a[i] && dp[i-1][x-a[i]][y]){
                        dp[i][x][y]=true;
                    }
                    if(y>=a[i] && dp[i-1][x][y-a[i]]){
                        dp[i][x][y]=true;
                    }
                }
            }
