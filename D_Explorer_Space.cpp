...
void solve(){
    ...
    vector<vector<int>>v(r-1,vector<int>(c));
    for(int i=0;i<r-1;i++){
        for(int j=0;j<c;j++){
            cin>>v[i][j];
        }
    ...
    vector<vector<vector<int>>>dp(r,vector<vector<int>>(c,vector<int>(k+1,1e18)));
            ...
            for(int j=0;j<c;j++){
                for(int dd=0;dd<4;dd++){
                    ...
                    if(new_i>=0 && new_j>=0 && new_i<r && new_j<c){
                        ...
                        dp[i][j][m]=min(dp[i][j][m],2*cost+dp[new_i][new_j][m-2]);
                    }
                }
            }
        }
