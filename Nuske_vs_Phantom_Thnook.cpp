...
void solve(){
    ...
    vector<vector<int>>hori(r+1,vector<int>(c+1,0));
    vector<vector<int>>ver(r+1,vector<int>(c+1,0));
    for(int i=1;i<=r;i++){
        for(int j=1;j<=c;j++){
            p[i][j]=p[i-1][j]+p[i][j-1]-p[i-1][j-1]+a[i][j];
            hori[i][j]=hori[i-1][j]+hori[i][j-1]-hori[i-1][j-1]+(a[i][j]==1 && a[i][j-1]==1);
            ver[i][j]=ver[i-1][j]+ver[i][j-1]-ver[i-1][j-1]+(a[i][j]==1 && a[i-1][j]==1);
        }
    }
    ...
    while(q--){
        int x1,y1,x2,y2;
        cin>>x1>>y1>>x2>>y2;
        int vertices=p[x2][y2]-p[x2][y1-1]-p[x1-1][y2]+p[x1-1][y1-1];
        int h_edges=hori[x2][y2]-hori[x2][y1]-hori[x1-1][y2]+hori[x1-1][y1];
        int v_edges=ver[x2][y2]-ver[x2][y1-1]-ver[x1][y2]+ver[x1][y1-1];
        int ans=vertices-h_edges-v_edges;
        cout<<ans<<endl;
    }
