...
int dfs(vector<vector<int>>&adj,set<int>&s,int i,int p,int &ans,int total){
    int val=s.count(i);
    for(auto &x:adj[i]){
        if(p!=x){
            int temp=dfs(adj,s,x,i,ans,total);
            ans+=min(temp,total-temp);
            val+=temp;
        }
    }
    return val;
