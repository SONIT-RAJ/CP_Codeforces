...
void solve(){
    ...
    int sq=sqrt(d*d-1);
    vector<vector<vector<int>>>dp(2,vector<vector<int>>(c+1,vector<int>(2,0)));
    for(int i=r;i>=1;i--){
        for(int j=0;j<=c;j++){
            dp[i%2][j][0]=0;
            dp[i%2][j][1]=0;
        ...
        for(int j=1;j<=c;j++){
            int right=min(c,sq+j);
            int left=max(0ll,j-sq-1);
            if(i==r){
                if(a[i][j]=='X'){
                    dp[i%2][j][0]=1;
                }
                ...
                if(a[i][j]=='X'){
                    int from0=(dp[(i+1)%2][right][0]-dp[(i+1)%2][left][0]+M)%M;
                    int from1=(dp[(i+1)%2][right][1]-dp[(i+1)%2][left][1]+M)%M;
                    dp[i%2][j][0]=(from0+from1)%M;
                }
                else{
                    dp[i%2][j][0]=0;
                }
            }
            dp[i%2][j][0]=(dp[i%2][j][0]+dp[i%2][j-1][0])%M;
        }
        for(int j=1;j<=c;j++){
            if(a[i][j]=='X'){
                int right=min(c,j+d);
                int left=max(j-d-1,0ll);
                dp[i%2][j][1]=(dp[(i)%2][right][0]-dp[(i)%2][left][0]+M)%M;
                int extra=(dp[(i)%2][j][0]-dp[(i)%2][j-1][0]+M)%M;
                dp[i%2][j][1]=(dp[(i)%2][j][1]-extra+M)%M;
            }
            else{
                dp[i%2][j][1]=0;
            }
            dp[i%2][j][1]=(dp[i%2][j][1]+dp[i%2][j-1][1])%M;
        }
    }
    cout<<(dp[1][c][0]+dp[1][c][1])%M;
