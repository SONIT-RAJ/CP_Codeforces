...
void solve(){
    ...
    int ans=0;
    for(int i=0;i<n;i++){
        int lefty=left.size() - left.order_of_key(a[i] + 1);
        auto it = right.find_by_order(right.order_of_key(a[i]));
        right.erase(it);
        int righty=right.order_of_key(a[i]);
        ans+=lefty*righty;
        left.insert(a[i]);
