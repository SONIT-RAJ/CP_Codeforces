...
// int dp[max_n];
void dfs(int node,int parent,vector<pair<int,int>>&a,vector<vector<int>>&adj,vector<vector<int>>&dp){
    for(auto &child:adj[node]){
        if(parent==child)continue;
        dfs(child,node,a,adj,dp);
        dp[0][node]+=max(dp[0][child]+abs(a[node].first-a[child].first),dp[1][child]+abs(a[node].first-a[child].second));
        dp[1][node]+=max(dp[0][child]+abs(a[node].second-a[child].first),dp[1][child]+abs(a[node].second-a[child].second));
    }
}
