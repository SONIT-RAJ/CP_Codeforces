...
bool check(vector<pair<int,int>>&a,vector<int>&q,int mid,int &n,int &m){
    vector<int>b(n+1,0);
    for(int i=0;i<=mid;i++){
        b[q[i]]=1;
    }
...
void solve(){
    ...
    if(check(a,q,high,n,m)){
        ans=high+1;
    }
    if(check(a,q,low,n,m)){
        ans=low+1;
    }
    cout<<ans;
