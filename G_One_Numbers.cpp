...
// int dp[max_n];
int dp[10][2][90];//index,tight,sum of digits

int f(string &s,int idx,int tight,int sum){
    if(idx>=s.size()){
        return isPrime(sum);
    }
    ...
    for(int i=0;i<=limit;i++){
        ans+=f(s,idx+1,tight&(i==limit),sum+i);
    }
...
void solve(){
    ...
    int rightAns=f(right,0,1,0);
    string left=to_string(l-1);
    memset(dp,-1,sizeof(dp));
    int leftAns=f(left,0,1,0);
