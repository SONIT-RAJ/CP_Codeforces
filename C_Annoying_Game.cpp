...
// int dp[max_n];
int f(vector<int>&a,vector<int>&b,vector<vector<vector<int>>>&dp,int k,int s,int i){
    if(i==a.size()){
        if(s==0){
            return -1e18;
        }
    ...
    if(s==2)return 0;
    if(dp[k][s][i]!=-1e18)return dp[k][s][i];
    int ans=-1e18;
    if(k==0){
        if(s==0){
            ans=max(ans,f(a,b,dp,0,0,i+1));
            ans=max(ans,a[i]+f(a,b,dp,0,1,i+1));
            ans=max(ans,a[i]+b[i]+f(a,b,dp,1,1,i+1));
            ans=max(ans,a[i]+b[i]+f(a,b,dp,1,2,i+1));
        }
        else{
            ans=max(ans,a[i]+f(a,b,dp,0,1,i+1));
            ans=max(ans,a[i]+b[i]+f(a,b,dp,1,1,i+1));
        ...
        ans=max(ans,a[i]+f(a,b,dp,1,1,i+1));
        ans=max(ans,a[i]+f(a,b,dp,1,2,i+1));
        ans=max(ans,f(a,b,dp,1,2,i+1));
    }
    return dp[k][s][i]=ans;
}
...
void solve(){
    ...
    vector<vector<vector<int>>>dp(2,vector<vector<int>>(3,vector<int>(n,-1e18)));
    cout<<f(a,b,dp,0,0,0);
