...
void solve(){
    ...
    int d2=query(min(n,1+d),max(1ll,1+d-(n-1)));
    if(d2==0){
        pri(min(n,1+d),max(1ll,1+d-(n-1)));
        return;
    }
    int d3=query(max(1ll,1+d-(m-1)),min(m,1+d));
    if(d3==0){
        pri(max(1ll,1+d-(m-1)),min(m,1+d));
        return;
    }
    int x=min(n,1+d);
    int y=max(1ll,1+d-(n-1));
    d2/=2;
    int d4=query(x-d2,y+d2);
    if(d4==0){
        pri(x-d2,y+d2);
    }
    else{
