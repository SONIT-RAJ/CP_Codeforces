...
bool dfs(vector<vector<int>>&a,vector<int>&v,int i,int f,int &zero,int &one){
    v[i]=f;
    if(f==0)zero++;
    else one++;
    for(auto &x:a[i]){
        if(v[x]==-1){
            if(dfs(a,v,x,1-f,zero,one))return true;
        }
        else{
            if(v[x]==f)return true;
        }
    }
    return false;
}
...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        ...
        int zero=0;
        int one=0;
        bool flag=dfs(a,v,i,0,zero,one);
        if(flag){
            cout<<0;
            return;
        }
        int temp=power(2,zero,M);
        temp=(temp+power(2,one,M))%M;
        ans=(ans*temp)%M;
    }
    cout<<ans;
