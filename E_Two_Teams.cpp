...
void func(int val,set<int>&t,vector<int>&ans,int k,int team,map<int,int>&idx){
    int index=idx[val];
    auto it = t.find(index);
    int left=k;
    while (left-- && it != t.begin()) {
...
void solve(){
    ...
    int f=0;
    for(auto &x:s){
        if(ans[idx[x]]!=0)continue;
        if(f==0){
            func(x,t,ans,k,1,idx);
        }
        else{
            func(x,t,ans,k,2,idx);
        }
        f=1-f;
    }
