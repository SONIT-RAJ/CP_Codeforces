    ...
    for(auto &x:mpp){
        ...
        if(sz%2==0){
            ...
            for (int i=sz-2;i>=0; i-=2){
                ...
                suff[i]=suff[i+2]+temp[i+1]-temp[i];
            }

            int mini=1e18;
            for (int i=0;i<sz;i+=2) {
                int left=(i-1<0)?0:pre[i-1];
                int right=(i+1>=sz)?0:suff[i+1];
