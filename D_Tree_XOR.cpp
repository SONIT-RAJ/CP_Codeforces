...
int dfs(vector<int>&a,vector<vector<int>>&adj,vector<int>&dp,int i,int p){
    int sz=1;
    int ans=0;
    for(auto &x:adj[i]){
        if(x==p)continue;
        ans+=dfs(a,adj,dp,x,i);
        sz+=dp[x];
    }
    dp[i]=sz;
    return ans+(sz*(a[i]^a[p]));
}
void dfs2(vector<int>&a,vector<vector<int>>&adj,vector<int>&ans,vector<int>&dp,int i,int p,int &n){
    ans[i]=ans[p]-(dp[i]*(a[i]^a[p]))+(n-dp[i])*(a[i]^a[p]);
    for(auto &x:adj[i]){
        if(x==p)continue;
        dfs2(a,adj,ans,dp,x,i,n);
    }
}
...
void solve(){
    ...
    a[0]=a[1];
    vector<int>ans(n+1);
    ans[1]=dfs(a,adj,dp,1,0);
    for(auto &x:adj[1]){
        dfs2(a,adj,ans,dp,x,1,n);
    }
    for(int i=1;i<=n;i++){
        cout<<ans[i]<<" ";
    }
