...
vector<int> sieve(int n) {
    vector<int> primes(n + 1, 1), res(n+1,1);
    primes[0] = primes[1] = 0;
    for (int i = 2; i <= n; i++) {
        if (primes[i]) {
            res[i]=i;
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        while(a[i]>1){
            ...
            mpp[prime].push_back(c);
        }
    }
    int ans=1;
    for(auto &key:mpp){
        int x=key.first;
        sort(mpp[x].begin(),mpp[x].end());
        int f=0;
        if(mpp[x].size()==n){
            f=mpp[x][1];
        }
        else if(mpp[x].size()==n-1){
            f=mpp[x][0];
        }
        while(f--){
            ans*=x;
