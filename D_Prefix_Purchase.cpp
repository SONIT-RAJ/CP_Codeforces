...
bool check(int val,int round,int rem,int mid,int temp){
    int total=val*round+rem;
    int cost=mid*temp+val*(round-mid);
...
void solve(){
    ...
    for(int i=1;i<n;i++){
        ...
        while(low<=high){
            ...
            if(check(val,round,rem,mid,temp)){
                req=mid;
                low=mid+1;
            }
            else{
        ...
        int cost=req*temp+val*(round-req);
        ans[idx+1]+=req;
        ans[a[i][1]+1]-=req;
        val=a[i][0];
        round=req;
