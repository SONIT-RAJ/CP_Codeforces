...
void solve(){
    ...
    vector<vector<int>>dp(n+1,vector<int>(h,-1e18));
    //dp[i][j]->maximum good upto ith index if start sleeping at j hr on ith sleep
    //ans = dp[n][y] = maximum among 0<=y<=h
    //dp[0][y]=0 for all 0<=y<=h
    // dp[i][j]=e+max(dp[i-1][j-a[i],dp[i-1][j-a[i]+1]])
    dp[0][0]=0;
    for(int i=1;i<=n;i++){
        for(int j=0;j<h;j++){
            int e=(l<=j && j<=r)?1:0;
            dp[i][j]=e+max(dp[i-1][(j-a[i]+h)%h],dp[i-1][(j-a[i]+1+h)%h]);
        }
    }
    int ans=0;
    for(int j=0;j<h;j++){
