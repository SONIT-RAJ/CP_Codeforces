...
int f(vector<int>&a,vector<int>&p,vector<int>&b,vector<int>&dp,int &k,int i){
    if(i>k){
        return 0;
    }
    ...
    for(int j=1;j+i-1<=k;j++){
        int cost=p[i+j-1]-p[i+b[j]-1];
        int temp=cost+f(a,p,b,dp,k,i+j);
        ans=min(ans,temp);
    }
    return dp[i]=ans;
}
...
void solve(){
  ...
  //  b[i] will give me maximum shovels that i can take for free if i have to buy exactly i shovels
 ...
 *
 * observations
 * 1. if i have to buy exactly k shovels then i pick the cheaptest of the k shovels from n shovels
 * 2. take that y that have maximum values for a particular x
 *
 */