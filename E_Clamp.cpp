...
void build(int low,int high,int index,vector<int>&a,vector<int>&seg){
    if(low==high){
        seg[index]=a[low];
    }
    else{
        build(low,(low+high)/2,2*index+1,a,seg);
        build((low+high)/2+1,high,2*index+2,a,seg);
        seg[index]=min(seg[2*index+1],seg[2*index+2]);
    }
}
int query(int l,int r,int low,int high,int index,vector<int>&seg){
    if(l<=low && r>=high){
        return seg[index];
    }
        ...
        return min(query(l,r,low,(low+high)/2,2*index+1,seg),query(l,r,(low+high)/2+1,high,2*index+2,seg));
    }
}
void point(int i,int val,int low,int high,int index,vector<int>&seg){
    if(low==high){
        if(low!=i){
            return;
        }
        ...
        int mid=(low+high)/2;
        if(mid>=i){
            point(i,val,low,mid,2*index+1,seg);
        }
        else{
            point(i,val,mid+1,high,2*index+2,seg);
        }
        seg[index]=min(seg[2*index+2],seg[2*index+1]);
    }
}
// ╭──────────────────────────────╮
// │        SOLVER ZONE           │
...
void solve(){
    ...
    while(q--){
        ...
        if(type==1){
            int i,val;
            cin>>i>>val;
            point(i,val,0,n-1,0,seg);
        }
        else{
            int l,r;
            cin>>l>>r;
            cout<<max(l,min(r,seg[0]))<<endl;
        }
    }
