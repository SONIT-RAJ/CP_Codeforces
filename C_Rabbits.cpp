...
int f(string &s,vector<vector<int>>&dp,int i,int left,int n){//0 means looking left
    ...
    if(left==0){
        ...
        if(s[i+1]=='0'){
            return dp[i][left]=(f(s,dp,i+1,0,n)||f(s,dp,i+1,1,n));
        }
        for(int j=i+2;j<n;j++){
            if(s[j]=='0'){
                return dp[i][left]=f(s,dp,j,1,n);
            }
        }
        return dp[i][left]=1;
        ...
        return dp[i][left]=f(s,dp,i+2,0,n);
    }
}
