...
pair<int,int> f(string &s,int idx,int tight,int start,int bit){
    if(idx==s.size()){
        if(start==1)return {0,0};
        return {1,0};
    }
    pair<int,int>temp={-1,-1};
    if(dp[idx][tight][start][bit]!=temp){
        return dp[idx][tight][start][bit];
    }
    ...
    for(int i=0;i<=limit;i++){
        ...
        if(new_start==0){
            new_bit=bit & ~(1 << i);
            int distinct=10-ppcll(new_bit);
            if(distinct>k)continue;
        }
        pair<int,int>tt=f(s,idx+1,new_tight,new_start,new_bit);
        int val=tt.first;
        int tempo=tt.second;
        if(new_start==0){
            sum=(sum+(val*(i*a[s.size()-idx-1])%M)%M)%M;
        }
        sum=(sum+tempo)%M;
        cnt+=val;
        cnt%=M;
    }
    return dp[idx][tight][start][bit]={cnt,sum};
}
...
void solve(){
    ...
    mem1(dp);
    int right=f(high,0,1,1,1023).second;
    mem1(dp);
    int left=f(low,0,1,1,1023).second;
    cout<<(right-left+M)%M;
