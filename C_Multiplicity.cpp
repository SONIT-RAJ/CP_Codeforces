...
void solve(){
    ...
    vector<int>dp(n+1,0);
    //  dp[i][j] = number of subsequence of length j upto ith position
    //  ans = dp[n-1][1] + dp[n-1][2] + ... + dp[n-1][n] (we excluded dp[n-1][0] as empty subsequence is not allowed according to question)
    // base case
    //  dp[0][0]=1
    //  dp[0][1]=1
    //  dp[0][2] = dp[0][3] = dp[0][4] =...dp[0][n] = 0
    //  dp[x][0]=1
    dp[0]=1;
    dp[1]=1;
    for(int i=1;i<n;i++){
