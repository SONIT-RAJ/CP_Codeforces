...
void solve(){
    ...
    vector<vector<int>>dp(n,vector<int>(3,2e18));
    dp[0][0]=0;
    dp[0][1]=b[0];
    dp[0][2]=2*b[0];
    for(int i=1;i<n;i++){
        for(int j=0;j<3;j++){
            int temp=2e18;
            int e=(a[i-1]==a[i]+j)?1e18:j*b[i];
            temp=min(temp,dp[i-1][0]+e);
            e=(a[i-1]+1==a[i]+j)?1e18:j*b[i];
            temp=min(temp,dp[i-1][1]+e);
            e=(a[i-1]+2==a[i]+j)?1e18:j*b[i];
            temp=min(temp,dp[i-1][2]+e);
