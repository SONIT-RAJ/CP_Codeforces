...
template <typename T> using sonit = tree<T, null_type, cmp, rb_tree_tag, tree_order_statistics_node_update>;
// ╭──────────────────────────────╮
// │        SOLVER ZONE           │
// ╰──────────────────────────────╯
void solve(){
...
typedef tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update> pbds; // find_by_order, order_of_key, increasing order
typedef tree<int, null_type, greater<int>, rb_tree_tag, tree_order_statistics_node_update> pbds; // find_by_order, order_of_key, decreasing order
typedef tree<int, null_type, less_equal<int>, rb_tree_tag, tree_order_statistics_node_update> pbds; // find_by_order, order_of_key,increasing order, equals are allowed

pbds a; -> how to declare


template <typename T> using pbds = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;//generic

pbds<int>a; -> how to declare


template <typename T> using pbds = tree<T, null_type, cmp, rb_tree_tag, tree_order_statistics_node_update>;// advanced
struct cmp {
    bool operator()(const vector<int>& a, const vector<int>& b) const {
        return a[0] < b[0];
    }
};
pbds<vector<int>>a; -> how to declare
    a.insert({1,2,3});
    a.insert({2,3,4});
    for(auto x:a){
        for(auto y:x){
