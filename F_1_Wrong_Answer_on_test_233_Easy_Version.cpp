...
int f(vector<int>&a,vector<vector<int>>&dp,int &n,int &k,int i,int s){
    if(i>=n){
        if(s-n>0)return 1;
        return 0;
    }
    ...
    int future=a[(i+1)%n];
    if(present==future){
        dp[i][s]=(k*f(a,dp,n,k,i+1,s))%M;
    }
    else{
        dp[i][s]=(f(a,dp,n,k,i+1,s+1)+f(a,dp,n,k,i+1,s-1))%M;
        int temp=((k-2)*f(a,dp,n,k,i+1,s))%M;
        dp[i][s]=(dp[i][s]+temp)%M;
    }
    return dp[i][s];
}
