...
int f(vector<int>&a,vector<int>&b,vector<vector<int>>&dp,int i,int j){
    if(i>=a.size()){
        return 0;
    }
    if(dp[j][i]!=-1)return dp[j][i];
    dp[j][i]=f(a,b,dp,i+1,0);
    if(b[i]==-1){
        return dp[j][i];
    }
    if(j==0){
        return dp[j][i]=max(dp[j][i],f(a,b,dp,b[i],1)+b[i]-i+1);
    }
    else{
        return dp[j][i]=max(dp[j][i],f(a,b,dp,b[i],1)+b[i]-i);
    }
}
...
void solve(){
    ...
    //tc - O(n)
    //sc - O(n)
    //aproach - like knapsack dp we have two choice either to take a segment or not
