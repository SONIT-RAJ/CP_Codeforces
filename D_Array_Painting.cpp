...
int f(vector<int>&a,vector<vector<int>>&dp,int i,int j){
    if(i>=a.size()){
        if(j==2 || j==3){
            return 1;
        }
    ...
    if(a[i]==0){
        if(j==0){
            return dp[i][j]=min(1+f(a,dp,i+1,0),f(a,dp,i+1,3));
        }
        else if(j==1){
            return dp[i][j]=f(a,dp,i+1,0);
        }
        else if(j==2){
            return dp[i][j]=min(1+f(a,dp,i+1,3),2+f(a,dp,i+1,0));
        }
        else{
            return dp[i][j]=min(1+f(a,dp,i+1,3),2+f(a,dp,i+1,0));
        }
        ...
        if(j==0){
            return dp[i][j]=min(1+f(a,dp,i+1,1),f(a,dp,i+1,2));
        }
        else if(j==1){
            return dp[i][j]=f(a,dp,i+1,1);
        }
        else if(j==2){
            return dp[i][j]=min(1+f(a,dp,i+1,0),f(a,dp,i+1,2));
        }
        else{
            return dp[i][j]=min(1+f(a,dp,i+1,0),f(a,dp,i+1,2));
        }
