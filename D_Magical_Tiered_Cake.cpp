...
void hanoi(int n,int from,int to,int help,vector<vector<int>>&ans,vector<int>&a){
    if(n==0){
        return;
    }
    if(n==1){
    ...
    int rem=n-1-a[n];
    hanoi(rem,from,help,to,ans,a);
    ans.push_back({n,from,to});
    if(a[n]==0){
        hanoi(n-1,help,to,from,ans,a);
    }
    else{
        hanoi(rem,help,from,to,ans,a);
        hanoi(n-1,from,to,help,ans,a);
    }
}
